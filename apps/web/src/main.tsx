...
const edgeNav = [
  ...
  ["__configuration", "CONFIGURAZIONE", null],
  ["plant", "Struttura impianto", Factory],
  ["communications", "Comunicazioni", Cable],
  ["catalog", "Catalogo modelli", BookOpen],
  ["__governance", "GESTIONE", null],
  ["compliance", "Conformità e incentivi", ClipboardCheck],
  ["sync", "Connessione Control Room", Cloud],
  ["__system", "AMMINISTRAZIONE", null],
  ["settings", "Preferenze", Settings],
  ["diagnostics", "Stato sistema", CircleGauge],
...
const pageRoles: Record<string, UserRole[]> = {
  ...
  kpi: everyone,
  alarms: everyone,
  plant: ["platform_admin", "technician"],
  ...
  diagnostics: ["platform_admin", "technician", "customer_admin"],
  users: ["platform_admin", "customer_admin"],
  tailscale: ["platform_admin", "technician"],
  audit: ["platform_admin", "technician"],
  customers: ["platform_admin"],
  sites: ["platform_admin", "technician"],
  fleet: ["platform_admin", "technician"],
  tree: ["platform_admin", "technician"],
  activations: ["platform_admin", "technician"],
  __operations: everyone,
...
const roleLabels: Record<string, string> = {
  ...
  technician: "Tecnico commissioning",
  customer_admin: "Amministratore",
  operator: "Operatore",
  viewer: "Visualizzatore",
};
const canAccess = (page: string, role?: string) =>
  !!role && (!pageRoles[page] || pageRoles[page].includes(role as UserRole));
...
function Login({ onLogin }: { onLogin: (token: string) => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      localStorage.setItem(`em-token-${mode}`, data.access_token);
  ...
  return (
    <main className="login">
      <section className="login-panel">
        ...
        <p className="eyebrow">ENERGY MANAGER</p>
        <h1>
          {mode === "edge" ? "Il tuo ecosistema energetico" : "Control Room"}
        </h1>
        <p className="muted">
          {mode === "edge"
            ? "Un solo luogo per fotovoltaico, accumulo, ricarica elettrica e consumi."
            : "Supervisione della flotta Edge e dei siti connessi."}
        </p>
        <form onSubmit={submit}>
          ...
          <button disabled={busy}>
            {busy ? "Accesso…" : "Entra nella tua energia"}
          </button>
        </form>
        <p className="login-foot">Elaborazione locale · Connessione protetta</p>
      </section>
      <section className="login-art">
        <div className="orb"></div>
        <div>
          <p className="eyebrow">ENERGY ECOSYSTEM</p>
          <h2>L’energia vive qui.</h2>
          <p>
            Produzione, accumulo, mobilità e consumi diventano un unico sistema,
...
function Status({ value }: { value: any }) {
  ...
  const ok =
    ["online", "good", "ok", "running", "closed", true].includes(value) ||
    ["online", "good", "ok", "running", "closed"].includes(text.toLowerCase());
  return (
    <span
      className={`status ${ok ? "ok" : text === "open" || text === "offline" ? "bad" : "warn"}`}
    >
      <i />
      {text}
    </span>
...
function Dashboard({
  ...
  const poly = points
    .map(
      (p: any, i: number) =>
        `${i * (700 / Math.max(points.length - 1, 1))},${160 - (Number(p.value) || 0)}`,
    )
    .join(" ");
  const meter = data.primary_meter;
  const subtitle =
    [data.site_name, meter && `${meter.manufacturer} ${meter.model}`]
      .filter(Boolean)
      .join(" · ") || "Edge locale";
  return (
    <>
      <Header
        ...
        action={
          <div className="head-actions">
            <button className="primary-button" onClick={onOpenPlant}>
              <Factory size={16} />
              Gestisci impianto
            </button>
            <Status value={data.devices_online ? "online" : "offline"} />
          </div>
        }
      />
      <div className="metrics">
        ...
        <Metric
          label="Picco periodo"
          value={data.peak_kw?.toFixed?.(1)}
          unit="kW"
          tone="amber"
        />
        <Metric
          label="Dispositivi online"
          value={`${data.devices_online ?? 0}/${data.devices_total ?? 0}`}
          tone="purple"
        />
      </div>
      <div className="grid-two">
        <Panel title="Andamento potenza" aside="Ultimi campioni">
          <div className="chart">
            <svg viewBox="0 0 700 190" preserveAspectRatio="none">
              <defs>
                <linearGradient id="fill" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0" stopColor="#2ed99b" stopOpacity=".3" />
                  <stop offset="1" stopColor="#2ed99b" stopOpacity="0" />
                </linearGradient>
              </defs>
              {poly && (
                <>
                  <polygon points={`0,190 ${poly} 700,190`} fill="url(#fill)" />
                  <polyline
                    points={poly}
                    fill="none"
                    stroke="#2ed99b"
            ...
            {!poly && (
              <div className="empty">In attesa dei dati dal simulatore</div>
            )}
          </div>
        </Panel>
        <Panel title="Stato operativo">
          <div className="signal-list">
            <div>
              <Wifi />
              <span>
                Qualità dati<small>Campioni normalizzati</small>
              </span>
              <Status value={data.quality} />
            </div>
            <div>
              <Cloud />
              <span>
                Sincronizzazione
                <small>{data.sync_pending ?? 0} eventi in coda</small>
              </span>
              <Status value={data.sync_pending ? "pending" : "ok"} />
            </div>
            <div>
              <ShieldCheck />
              <span>
                Tailscale<small>{data.tailscale?.ip || "Provider fake"}</small>
              </span>
              <Status
                value={data.tailscale?.connected ? "online" : "offline"}
              />
            </div>
            <div>
              <AlertTriangle />
              <span>
                Allarmi aperti<small>Soglie e comunicazioni</small>
              </span>
              <b>{data.open_alarms ?? 0}</b>
            </div>
          </div>
        </Panel>
      </div>
...
const endpointMap: Record<string, string> = {
  ...
  alarms: "/alarms",
  sync: "/sync/status",
  tailscale: "/tailscale/diagnostics",
  users: "/users",
  audit: "/audit",
  fleet: "/fleet",
  customers: "/fleet",
...
const titles: Record<string, string> = Object.fromEntries(
  [...(edgeNav as any), ...(controlNav as any)].map((x: any) => [x[0], x[1]]),
);
...
function DataPage({
  ...
  const load = () => {
    setError("");
    request(endpointMap[page] || "/health", token)
      .then(setData)
      .catch((e) => setError(e.message));
  };
  useEffect(load, [page, token]);
  const technical = ["platform_admin", "technician"].includes(role),
    administrative = [
      "platform_admin",
      "technician",
      "customer_admin",
  ...
  if (mode === "edge" && page === "settings")
    return <SystemCenter token={token} focus="preferences" />;
  if (mode === "edge" && page === "diagnostics")
    return <SystemCenter token={token} focus="status" />;
  if (mode === "edge" && page === "live")
    return (
      <div
        className={`${technical ? "" : "alarm-observer"} ${role === "viewer" ? "viewer-only" : ""}`}
      >
        <EdgeOperations token={token} focus="live" />
      </div>
    );
  if (mode === "edge" && page === "monitoring")
    return (
      <div className={administrative ? "" : "read-only"}>
  ...
  if (mode === "edge" && page === "kpi")
    return (
      <div className={technical ? "" : "read-only"}>
        <KpiStudio token={token} />
      </div>
    );
  if (mode === "edge" && page === "alarms")
    return (
      <div
        className={`${technical ? "" : "alarm-observer"} ${role === "viewer" ? "viewer-only" : ""}`}
      >
  ...
  if (page === "fleet")
    return (
      <>
        <Header
          title="Flotta Edge"
          subtitle="Stato aggregato per cliente e sito"
        />
        <Fleet token={token} />
      </>
    );
  if (page === "catalog") return <Catalog token={token} />;
  return (
    <>
      <Header
        title={titles[page] || page}
        subtitle="Vista operativa Energy Manager"
        action={
...
function DataView({ data }: { data: any }) {
  if (data === null) return <div className="loading">Caricamento…</div>;
  if (Array.isArray(data)) {
    ...
    const keys = Object.keys(data[0])
      .filter(
        (k) =>
          !["password_hash", "token_hash", "config", "definition"].includes(k),
      )
      .slice(0, 8);
    return (
...
function Assets({ token }: { token: string }) {
  const [items, setItems] = useState<any[]>([]);
  useEffect(() => {
    request("/assets", token).then(setItems);
  }, [token]);
  const roots = items.filter((x) => !x.parent_id);
  const branch = (node: any, depth = 0) => (
    <React.Fragment key={node.id}>
      <div className="tree-row" style={{ paddingLeft: 16 + depth * 28 }}>
      ...
      {items
        ...
        .map((x) => branch(x, depth + 1))}
    </React.Fragment>
  );
  return (
    <>
      <Header
        title="Albero energetico"
        subtitle="Separato dalla topologia di comunicazione"
      />
      <Panel title="Stabilimento Demo">
        <div className="tree">{roots.map((x) => branch(x))}</div>
      </Panel>
    </>
  );
}
function Catalog({ token }: { token: string }) {
  ...
  const load = () => request("/catalog", token).then(setItems);
  useEffect(() => {
    void load();
  }, [token]);
  async function upload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      await request("/catalog/import", token, { method: "POST", body: form });
      setMessage("Profilo validato e importato");
      load();
    } catch (err: any) {
  ...
  return (
    <>
      <Header
        title={mode === "edge" ? "Catalogo Modbus" : "Catalogo centralizzato"}
        subtitle="Profili indipendenti dalle istanze installate"
        action={
      ...
      <div className="catalog-grid">
        {items.map((p) => (
          <article className="catalog-card" key={p.id}>
            ...
            <p className="catalog-category">{categoryLabel(p.category)}</p>
            {p.description && <p>{p.description}</p>}
            <div
              className="catalog-tags"
              aria-label="Compatibilità del profilo"
            >
              {(p.capabilities?.components || []).map((component: string) => (
                <span key={component}>{componentLabel(component)}</span>
              ))}
...
function Plant({
  ...
  const [siteName, setSiteName] = useState(""),
    ...
    [device, setDevice] = useState<any>(deviceBlank),
    [asset, setAsset] = useState<any>({
      name: "",
      parent_id: "",
      category: "asset",
    }),
    [binding, setBinding] = useState<any>({
      asset_id: "",
      device_id: "",
  ...
  const load = async () => {
    try {
      const result = await request("/plant", token);
      setData(result);
  ...
  async function saveSite(e: React.FormEvent) {
    e.preventDefault();
    try {
      await request("/plant/site", token, {
        method: "PUT",
        body: JSON.stringify({ name: siteName }),
      });
      notify("Nome impianto aggiornato");
      await load();
    } catch (e) {
      fail(e);
    }
  ...
  async function addConnection(e: React.FormEvent) {
    ...
    const config =
      connection.kind === "modbus_tcp"
        ? {
            ...
            retry: Number(connection.retry),
          }
        : {
            port: connection.serial_port,
            baud_rate: Number(connection.baud_rate),
    ...
    try {
      ...
      setConnection(connectionBlank);
      setEditingConnectionId("");
      notify(
        editingConnectionId ? "Connessione aggiornata" : "Connessione aggiunta",
      );
      await load();
    } catch (e) {
      fail(e);
  ...
  function editConnection(c: any) {
    setEditingConnectionId(c.id);
    setConnection({
      name: c.name,
      kind: c.kind,
      host: c.config.host || "",
      port: c.config.port || 502,
      serial_port: c.config.port || "/dev/ttyUSB0",
      baud_rate: c.config.baud_rate || 9600,
      parity: c.config.parity || "N",
      timeout: c.config.timeout ?? 5,
      retry: c.config.retry ?? 0,
    });
  }
  async function addDevice(e: React.FormEvent) {
    ...
    try {
      await request(
        editingDeviceId ? `/devices/${editingDeviceId}` : "/devices",
        token,
        {
      ...
      setEditingDeviceId("");
      notify(
        editingDeviceId ? "Dispositivo aggiornato" : "Dispositivo aggiunto",
      );
      await load();
    } catch (e) {
      fail(e);
  ...
  function editDevice(d: any) {
    const p = data.profiles.find((item: any) => item.id === d.profile_id);
    ...
    if (p) {
      ...
      setDeviceManufacturer(p.definition?.manufacturer || "Siemens");
    }
  }
  async function addAsset(e: React.FormEvent) {
    e.preventDefault();
    try {
      await request("/assets", token, {
        method: "POST",
        body: JSON.stringify({
          ...asset,
          parent_id: asset.parent_id || null,
          description: "",
          sort_order: data.assets.length,
          active: true,
        }),
      });
      setAsset({ ...asset, name: "" });
      notify("Nodo energetico aggiunto");
      await load();
    } catch (e) {
      fail(e);
    }
  ...
  async function addBinding(e: React.FormEvent) {
    e.preventDefault();
    try {
      await request("/bindings", token, {
        method: "POST",
        body: JSON.stringify(binding),
      });
      notify("Misura associata al nodo");
      await load();
    } catch (e) {
      fail(e);
    }
  ...
  async function removeBinding(item: any) {
    try {
      await request(`/bindings/${item.id}`, token, { method: "DELETE" });
      notify("Associazione rimossa");
      await load();
    } catch (e) {
      fail(e);
    }
  ...
  const points = Array.from(
    new Set(
      data.profiles.flatMap((p: any) =>
        [
          ...(p.definition?.points || []),
          ...(p.definition?.derived_points || []),
        ].map((x: any) => x.key),
      ),
    ),
  ) as string[];
  const selectedConnection = data.connections.find(
    (c: any) => c.id === device.connection_id,
  );
  const categories = Array.from(
    new Set(
      data.profiles.map((p: any) => p.definition?.category).filter(Boolean),
    ),
  ) as string[];
  const manufacturers = Array.from(
    new Set(
      data.profiles
        .filter((p: any) => p.definition?.category === deviceCategory)
        .map((p: any) => p.definition?.manufacturer)
        .filter(Boolean),
    ),
  ) as string[];
  ...
  const tree = (node: any, depth = 0): React.ReactNode => (
    <React.Fragment key={node.id}>
      <div className="tree-row" style={{ paddingLeft: 16 + depth * 28 }}>
        <ChevronRight size={15} />
        <span className="asset-icon">
        ...
        <b>{node.name}</b>
        <small>
          {data.bindings.filter((b: any) => b.asset_id === node.id).length}{" "}
          misure · {node.category}
        </small>
      </div>
      {data.assets
        .filter((x: any) => x.parent_id === node.id)
        .map((x: any) => tree(x, depth + 1))}
    </React.Fragment>
  );
  const progress = [
    { id: "overview", label: "Identità", done: !!data.site },
    { id: "devices", label: "Dispositivi", done: data.devices.length > 0 },
    {
      id: "energy",
      label: "Albero energetico",
      done: data.assets.length > 0 && data.bindings.length > 0,
    },
  ];
  return (
    <>
      <Header
        title="Struttura impianto"
      ...
      {message && <div className="notice">{message}</div>}
      {error && <div className="alert">{error}</div>}
      <div className="provisioning-intro">
        <span>CONFIGURAZIONE GUIDATA</span>
        <b>Costruisci il modello energetico dell’impianto</b>
        <p>
          I canali Ethernet e seriali si gestiscono nella sezione Comunicazioni.
          Qui installi i dispositivi e li collochi nell’albero monte/valle.
        </p>
      </div>
      <div className="setup-progress three">
        {progress.map((step, i) => (
          <button
            key={step.id}
          ...
          >
            <span>{step.done ? <CheckCircle2 /> : i + 1}</span>
            <div>
              <b>{step.label}</b>
              <small>{step.done ? "Configurato" : "Da completare"}</small>
            </div>
          </button>
        ))}
      </div>
      {tab === "overview" && (
        <div className="plant-grid">
          <Panel title="Identità dell’impianto">
            <form className="form-stack" onSubmit={saveSite}>
              <label>
                Nome impianto
                <input
                  value={siteName}
            ...
            <div className="plant-summary">
              <div>
                <Network />
                <span>
                  <b>{data.connections.length}</b> canali dati
                </span>
              </div>
              <div>
                <Cable />
                <span>
                  <b>{data.devices.length}</b> dispositivi
                </span>
              </div>
              <div>
                <Factory />
                <span>
                  <b>{data.assets.length}</b> utenze / nodi
                </span>
              </div>
              <div>
                <Link2 />
                <span>
                  <b>{data.bindings.length}</b> misure associate
                </span>
              </div>
            </div>
          </Panel>
          <Panel title="Percorso consigliato">
            <div className="guide">
              <button onClick={() => onNavigate("communications")}>
                <span>1</span>
                <div>
                  <b>Prepara le comunicazioni</b>
                  <small>
                    Rete Edge, Modbus TCP e porte RS485 in una sezione dedicata.
                  </small>
                </div>
                <ChevronRight />
              </button>
              <button onClick={() => setTab("devices")}>
                <span>2</span>
                <div>
                  <b>Installa i dispositivi</b>
                  <small>
                    Scegli modello e indirizzo oppure usa la ricerca automatica.
                  </small>
                </div>
                <ChevronRight />
              </button>
              <button onClick={() => setTab("energy")}>
                <span>3</span>
                <div>
                  <b>Disegna l’albero energetico</b>
                  <small>
                    Dal contatore generale fino a linee, reparti e macchine.
                  </small>
                </div>
                <ChevronRight />
              </button>
      ...
      {tab === "connections" && (
        <div className="plant-grid">
          <Panel title="Connessioni configurate">
            {data.connections.length ? (
                    ...
                    <div>
                      ...
                      <small>
                        {c.kind === "modbus_tcp"
                          ...
                          : `${c.config.port} · ${c.config.baud_rate} baud`}{" "}
                        · timeout {c.config.timeout ?? 2}s · retry{" "}
                        {c.config.retry ?? 0}
                      </small>
                    </div>
              ...
              <div className="empty">Nessuna connessione configurata</div>
            )}
          </Panel>
          <Panel
            title={
              editingConnectionId ? "Modifica connessione" : "Nuova connessione"
            }
          >
            <form className="form-grid" onSubmit={addConnection}>
              <label className="wide">
                ...
                <input
                  ...
                  placeholder="Gateway quadro generale"
                  required
                />
              </label>
              <label>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setConnection({ ...connection, kind: e.target.value })
                  }
                >
                ...
                <>
                  ...
                  <label>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setConnection({ ...connection, parity: e.target.value })
                      }
                    >
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setConnection({ ...connection, timeout: e.target.value })
                  }
                />
              </label>
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setConnection({ ...connection, retry: e.target.value })
                  }
                />
              </label>
              <p className="model-help wide">
                L’Edge mantiene una sola sessione TCP per gateway e interroga
                gli slave in sequenza. Su server lenti usa un timeout adeguato;
                evita retry elevati che possono creare risposte arretrate.
              </p>
      ...
      {tab === "energy" && (
        <>
          ...
          />
          <div className="plant-grid">
            <Panel title="Misure associate">
              {data.bindings.length ? (
                <div className="binding-list">
                  {data.bindings.map((b: any) => (
                    <div key={b.id}>
                      <Link2 />
                      <span>
                        <b>{b.asset_name}</b>
                        <small>
                          {b.device_name} · {b.measurement_key}
                        </small>
                      </span>
                      <em>{b.role}</em>
                      <button
                ...
                <div className="empty">
                  Nessuna misura associata. Collega il contatore generale al
                  nodo radice e poi procedi verso le utenze a valle.
                </div>
              )}
            </Panel>
            <Panel title="Colloca una misura nell’albero">
              <form className="form-grid" onSubmit={addBinding}>
                <label className="wide">
                  Dove viene misurata?
                  <select
                    value={binding.asset_id}
                    onChange={(e) =>
                      setBinding({ ...binding, asset_id: e.target.value })
                    }
                    required
                  >
                    <option value="">Seleziona nodo / utenza…</option>
                ...
                <label className="wide">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setBinding({ ...binding, device_id: e.target.value })
                    }
                    required
                  >
                ...
                <label className="wide">
                  Grandezza normalizzata
                  <select
                    value={binding.measurement_key}
                    onChange={(e) =>
                      setBinding({
                        ...binding,
                        measurement_key: e.target.value,
                      })
                    }
                  >
                    {points.map((key) => (
      ...
      {removingDevice && (
        <DeviceRemoval
          ...
          onRemoved={async () => {
            await load();
            notify("Dispositivo rimosso dalla configurazione");
          }}
        />
      )}
    </>
...
function App() {
  const [token, setToken] = useState(
      localStorage.getItem(`em-token-${mode}`) || "",
    ),
  ...
  useEffect(() => {
    if (token)
      request("/me", token)
        .then(setMe)
        .catch(() => {
          localStorage.removeItem(`em-token-${mode}`);
          setToken("");
        });
  }, [token]);
  useEffect(() => {
    if (me?.ui_preferences?.theme) applyTheme(me.ui_preferences.theme);
  }, [me]);
  useEffect(() => {
    if (me && !canAccess(page, me.role)) setPage("dashboard");
  }, [me, page]);
  ...
  if (!me)
    return (
      <div className="session-loader">
        <span />
        <b>Preparazione della tua esperienza…</b>
      </div>
    );
    ...
    <div className="app">
      <aside className={mobile ? "open" : ""}>
        <div className="brand">
          ...
          <div>
            <b>Energy Manager</b>
            <small>{mode === "edge" ? "ENERGY OS" : "CONTROL ROOM"}</small>
          </div>
        ...
        <div className="profile">
          ...
          <span>
            <b>{me.username}</b>
            <small>{roleLabels[me.role] || me.role}</small>
          </span>
